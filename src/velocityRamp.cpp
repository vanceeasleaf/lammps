...
VelocityRamp::VelocityRamp(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

void VelocityRamp::command(int narg, char **arg)
{
  if (narg < 2) error->all(FLERR,"Illegal velocity command");

  if (domain->box_exist == 0)
    error->all(FLERR,"VelocityRamp command before simulation box is defined");
  if (atom->natoms == 0)
    error->all(FLERR,"VelocityRamp command with no atoms existing");
  ...
  if (style == CREATE) options(narg-10,&arg[10]);
  ...
  if (style == CREATE) {
    ...
    int seed = atoi(arg[3]);
    create(t_desired,seed,narg-4,&arg[4]);
  }
...
void VelocityRamp::init_external(const char *extgroup)
{
...
void VelocityRamp::create(double t_desired, int seed,int narg, char **arg)
{
  int i;
...
// set scale factors
  ...
  if (v_dim == 2 && domain->dimension == 2)
    error->all(FLERR,"VelocityRamp ramp in z for a 2d problem");
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      v[i][0]*=sqrt(vramp/t_desired);
      v[i][1]*=sqrt(vramp/t_desired);
      v[i][2]*=sqrt(vramp/t_desired);	 
    }
...
void VelocityRamp::set(int narg, char **arg)
{
...
void VelocityRamp::scale(int narg, char **arg)
{
...
void VelocityRamp::ramp(int narg, char **arg)
{
  // set scale factors
...
void VelocityRamp::zero(int narg, char **arg)
{
...
void VelocityRamp::rescale(double t_old, double t_new)
{
...
void VelocityRamp::zero_momentum()
{
...
void VelocityRamp::zero_rotation()
{
  int i;
...
void VelocityRamp::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"dist") == 0) {
      ...
      if (temperature->tempflag == 0)
        error->all(FLERR,
                   "VelocityRamp temperature ID does not compute temperature");
      iarg += 2;
