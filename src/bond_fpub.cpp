...
BondFPUB::BondFPUB(LAMMPS *lmp) : Bond(lmp) {}
...
void BondFPUB::compute(int eflag, int vflag)
{
  int i1,i2,n,type;
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if(d<1e-8)e=0.0;
    if(c<0.0)e=-1.0;
    double u=(k[type]*d+beta[type]*d*d*d+g[type]*rv)*e/rp;
    double fx=-u*delx;
    double fy=-u*dely;
    e=1.0;
    if(rv<1e-8)e=0.0;
    if(delz<0.0)e=-1.0;
    double fz=-(k[type]*rv+beta[type]*rv*rv*rv+g[type]*d)*e;
    // force & energy

    fbond = -u;

    if (eflag) ebond = 1.0*(.5*k[type] *(d*d+rv*rv) +1.0/4.0*beta[type]*(d*d*d*d+rv*rv*rv*rv)+g[type]*rv*d);
...
void BondFPUB::coeff(int narg, char **arg)
{
...
double BondFPUB::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondFPUB::write_restart(FILE *fp)
{
...
void BondFPUB::read_restart(FILE *fp)
{
  allocate();
...
void BondFPUB::write_data(FILE *fp)
{
...
double BondFPUB::single(int type, double rsq, int i, int j,
                        double &fforce)
{
  double r = sqrt(rsq);
  double dr = r - r0[type];
  ...
  return .5*k[type] * dr *dr+ .25*beta[type]* dr*dr*dr*dr;
}
